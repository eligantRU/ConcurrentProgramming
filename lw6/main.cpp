...
Bitmap BlurBitmap(const Bitmap & bmp, LogBuffer & logger, clock_t beginTime, size_t threadNum)
{
...
void Blur(std::string_view imgInName, std::string_view imgOutName, size_t threadsCount, size_t coresCount, const std::vector<ThreadPriority> & threadPriorities)
{
	...
	LogFileWriter logWriter("log.txt");
	LogBuffer logger(logWriter);
	...
	for (size_t i = 0; i < subBitmaps.size(); ++i)
	{
		threads.emplace_back([&subBitmaps, &logger, beginTime, i]() {
			subBitmaps[i] = BlurBitmap(subBitmaps[i], logger, beginTime, i);
		});
